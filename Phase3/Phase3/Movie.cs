...
public class Movie : IMovie
{
    ...
    {
        if(this.availablecopies > 0 && !Borrowers.Search(member))// checks that movie can be borrowed
        {
            Borrowers.Add(member); // adding member to borrower list
            this.availablecopies--; // decreasing the number of available movies
            this.noborrows++; // increasing number of times the movie has been borrowed 
            Console.WriteLine("Movie has been borrowed");
            return true;
        }
        Console.WriteLine("not enough copies or borrower on list");
        return false;
    }
    ...
    {
        //return this.Title.ToString() + ", " + this.genre.ToString() + ", " + this.Classification.ToString() + ", " + this.duration.ToString() + ", " + this.AvailableCopies.ToString();
        return string.Format("Movie: {0}, Genre: {1}, Classification: {2}, Duration: {3}, Available Copies: {4}", Title, genre, Classification, duration, AvailableCopies);
    }
